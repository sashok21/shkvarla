...
print("Маршрути поїздів:")
for route in routes:
    print(f"ID: {route[0]}, Номер: {route[1]}, Відправлення: {route[2]}, Прибуття: {route[3]}, Час: {route[4]} - {route[5]}")

cursor.execute("SELECT * FROM train_cars")
cars = cursor.fetchall()
print("\nВагони поїздів:")
for car in cars:
    print(f"ID: {car[0]}, Поїзд: {car[1]}, Тип: {car[2]}, Кількість місць: {car[3]}")

cursor.execute("SELECT * FROM train_tickets")
tickets = cursor.fetchall()
print("\nКвитки на поїзди:")
for ticket in tickets:
    print(f"ID: {ticket[0]}, Поїзд: {ticket[1]}, Вагон ID: {ticket[2]}, Пасажир: {ticket[3]}, Місце: {ticket[4]}, Ціна: {ticket[5]:.2f} грн")
